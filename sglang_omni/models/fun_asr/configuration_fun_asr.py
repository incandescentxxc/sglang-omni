...
class FunAsrNanoFeatureExtractor(SequenceFeatureExtractor):
    ...
    def _lfr(self, fbank: torch.Tensor) -> tuple[torch.Tensor, int]:
        ...
        Stacks ``num_frames_lfr`` frames every ``stride_lfr`` stride: left-pad by
        repeating the first frame ``(num_frames_lfr-1)//2`` times, right-pad the
        last frame to fill the final window, then gather via ``as_strided``.
